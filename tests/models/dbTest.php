...
class DbTest extends TestCase {
	...
	public function test_table_update() {
		$results = static::$db->wrap_table_update( 'technote_test_table1' );
		$this->assertNotEmpty( $results );
		$results = static::$db->wrap_table_update( 'technote_test_table2' );
		$this->assertNotEmpty( $results );
	}
	...
	public function test_column_check1() {
		...
		$columns = array_combine( array_map( function ( $column ) {
			return $column['Field'];
		}, $columns ), $columns );
	...
	public function test_insert() {
		...
		$this->assertEquals( 2, static::$db->builder()->table( 'technote_test_table1' )->insert( [
			'value1' => 'text10',
			'value2' => 10,
			'value3' => 'text30',
		] ) );
		$this->assertEquals( 5, static::$db->builder()->table( 'technote_test_table2' )->insert( [
			'value2' => 0,
			'value3' => 'text10',
			'value4' => 10,
		] ) );
		$this->assertEquals( 6, static::$db->builder()->table( 'technote_test_table2' )->insert( [
			'value2' => 100,
			'value3' => 'text20',
			'value4' => 20,
		] ) );
		$this->assertEquals( 7, static::$db->builder()->table( 'technote_test_table2' )->insert( [
			'value2' => 200,
			'value3' => 'text20',
			'value4' => null,
		] ) );
		$this->assertEquals( 8, static::$db->builder()->table( 'technote_test_table2' )->insert( [
			'value2' => 300,
			'value3' => 'text20',
			'value4' => 2,
		] ) );
	...
	public function test_update() {
		$this->assertEquals( 1, static::$db->update(
			'technote_test_table2',
			[
				'value3' => 'text3',
				'value4' => 3,
			],
		...
		$this->assertEquals( 0, static::$db->update(
			'technote_test_table2',
			[
				'value3' => 'text4',
				'value4' => 4,
		...
		$this->assertEquals( 1, static::$db->builder()->table( 'technote_test_table2' )->where( 'id', 5 )->update( [
			'value3' => 'text30',
			'value4' => 30,
		] ) );
		$this->assertEquals( 0, static::$db->builder()->table( 'technote_test_table2' )->where( 'id', 10 )->update( [
			'value3' => 'text40',
			'value4' => 40,
		] ) );
		$this->assertEquals( 1, static::$db->builder()->table( 'technote_test_table2' )->where( 'id', 8 )->update( [
			'value4' => null,
		] ) );
	}
	...
	public function test_select() {
		...
		$result = static::$db->builder()->table( 'technote_test_table2' )->find( 5 );
		$this->assertArrayHasKey( 'id', $result );
		$this->assertArrayHasKey( 'value3', $result );
		$this->assertArrayHasKey( 'value4', $result );
		...
		$results = static::$db->builder()->table( 'technote_test_table2' )->where_integer_in_raw( 'id', [ 7, 8 ] )->get();
		$this->assertCount( 2, $results );
		$this->assertNull( $results[0]['value4'] );
		$this->assertNull( $results[1]['value4'] );
	}
	...
	public function test_select2() {
		...
		$results = static::$db->builder()->table( 'technote_test_table2' )->find( 10 );
		$this->assertEmpty( $results );
	}
	...
	public function test_count() {
		$this->assertEquals( 2, static::$db->select_count( 'technote_test_table1' ) );
		$this->assertEquals( 8, static::$db->select_count( 'technote_test_table2' ) );
		$this->assertEquals( 1, static::$db->select_count( 'technote_test_table1', '*', [ 'value2' => 1 ] ) );

		$this->assertEquals( 2, static::$db->builder()->table( 'technote_test_table1' )->count() );
		$this->assertEquals( 8, static::$db->builder()->table( 'technote_test_table2' )->count() );
		$this->assertEquals( 1, static::$db->builder()->table( 'technote_test_table1' )->where( 'value2', 1 )->count() );
	}
	...
	public function test_chunk() {
		$count = 0;
		static::$db->builder()->table( 'technote_test_table2' )->chunk( 2, function ( $results ) use ( &$count ) {
			$this->assertCount( 2, $results );
			$count++;
		} );
	...
	public function test_each() {
		$count = 0;
		static::$db->builder()->table( 'technote_test_table2' )->each( 2, function () use ( &$count ) {
			$count++;
		} );
	...
	public function test_delete() {
		...
		$this->assertEquals( 1, static::$db->builder()->table( 'technote_test_table1' )->delete( 2 ) );
		$this->assertEquals( false, static::$db->builder()->table( 'technote_test_table1' )->delete( 2 ) );
		$this->assertEquals( 1, static::$db->builder()->table( 'technote_test_table2' )->delete( 5 ) );
		$this->assertEquals( 1, static::$db->builder()->table( 'technote_test_table2' )->delete( 6 ) );
		$this->assertEquals( false, static::$db->builder()->table( 'technote_test_table2' )->delete( 10 ) );
		$this->assertEquals( 2, static::$db->builder()->table( 'technote_test_table2' )->where_integer_in_raw( 'id', [ 7, 8 ] )->delete() );
	}
	...
	public function test_select3() {
		...
		$results = static::$db->builder()->table( 'technote_test_table1' )->where( 'id', 1 )->get();
		$this->assertEmpty( $results );
		$result = static::$db->builder()->table( 'technote_test_table2' )->find( 1 );
